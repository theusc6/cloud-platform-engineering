...
def create_firehose_policy(aws_params, key_alias, lambda_arn):
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [
                    ...
                    f"/myorg-splunk-cloudtrail-processor-firehose-{account_id}-{region}"
                ]
            },
            {
                "Sid": "Cloudwatch",
    ...
    policy_name = f"policy-myorg-splunk-cloudtrail-processor-{account_id}-{region}"
...
def create_firehose_role(session, account_id, region, custom_policy_arn):
    ...
    role_name = f'role-myorg-splunk-cloudtrail-processor-{account_id}-{region}'
    trust_policy = {
        "Version": "2012-10-17",
        "Statement": [
            {
...
def create_firehose_stream(aws_params, endpoint_url, hec_token, lambda_arn, role_arn):
    ...
    stream_name = f"myorg-splunk-cloudtrail-processor-firehose-{account_id}-{region}"
...
def create_cloudwatch_policy(aws_params):
    ...
    policy_name = f"policy-myorg-splunk-cloudwatch-destination-{account_id}-{region}"
...
def create_cloudwatch_role(session, account_id, region, custom_policy_arn):
    ...
    """
    # Constants for the role and policies
    role_name = f'role-myorg-splunk-cloudwatch-destination-{account_id}-{region}'
    trust_policy = {
        "Version": "2012-10-17",
        "Statement": [
            {
                ...
                "Condition": {
                    "StringLike": {
                        "aws:SourceArn": [
                            f"arn:aws:logs:{region}:123456789012:*",
...
def create_cloudwatch_logs_destination(session, account_id, region, firehose_arn,
    ...
    destination_name=f"myorg-splunk-cloudtrail-processor-destination-{account_id}-{region}"
...
def main():
    ...
    names = {
        "bucket_name": f'myorg-splunk-cloudtrail-processor-bucket-{args.account_id}-{args.region}',
        "key_alias": f'alias/myorg-splunk-cloudtrail-processor-key-{args.account_id}-{args.region}',
        "function_name": f"myorg-splunk-cloudtrail-processor-function-{args.account_id}-{args.region}"
    }
