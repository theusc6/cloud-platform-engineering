...
def create_iam_policy(aws_params, key_alias):
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [
                    ...
                    f"/myorg-splunk-guardduty-processor-firehose-{account_id}-{region}"
                ]
            },
            {
                "Sid": "Cloudwatch",
    ...
    policy_name = f"policy-myorg-splunk-guardduty-processor-{account_id}-{region}"
...
def create_firehose_role(session, account_id, region, custom_policy_arn):
    ...
    role_name = f'role-myorg-splunk-guardduty-processor-{account_id}-{region}'
    trust_policy = {
        "Version": "2012-10-17",
        "Statement": [
            {
...
def create_guard_duty_eventbridge_rule(session, account_id, region):
    ...
    guard_duty_rule_name = f'myorg-splunk-guardduty-rule-{account_id}-{region}'
...
def create_firehose_stream(aws_params, endpoint_url, hec_token, role_arn):
    ...
    stream_name = f"myorg-splunk-guardduty-processor-firehose-{account_id}-{region}"
...
def main():
    ...
    names = {
        "bucket_name": f'myorg-splunk-guardduty-processor-bucket-{args.account_id}-{args.region}',
        "key_alias": f'alias/myorg-splunk-guardduty-processor-key-{args.account_id}-{args.region}',
