"""
AWS Security Notifier Service Setup Script

This script automates the setup for the AWS security notifier service, which aims to notify
security staff of Guard Duty and select Security Hub findings. 

The script performs the following tasks:

- Sets up an IAM role for various AWS services like Lambda and SNS.
- Creates a KMS key for data encryption.
- Provisions a Lambda function for processing the security notifications.
- Configures an EventBridge rule to schedule regular security checks.
- Assigns appropriate triggers and permissions for the Lambda function.

Key functionalities include:
1. Role creation for security services with necessary permissions.
2. KMS key creation for data encryption and decryption.
3. Lambda function deployment for processing and sending notifications.
4. Scheduler rule setup in EventBridge for recurring security checks.
5. Assignment of EventBridge rule as a trigger for the Lambda function.*

*This only works as a rule, not a schedule. This script is using a schedule. 
The function for using a rule is provided however in the event it is needed.

*The lambda code still needs to be placed within the function after it has been
created.

Usage:
    To use this script, provide the AWS profile, region, and account_id as command-line arguments.
    Example: `python script_name.py --profile your_profile --region your_region
...
def create_function_role(session, account_id):
    """
    This function creates a role for SNS so that it can be utilized by and 
    with the security notifier service. This service will notify security staff of 
    Guard Duty and select Security Hub findings. 
    ...
    role_name = 'role-myorg-securityhub-findings-report+prod-ops'
            ...
            },
            {
                ...
                "Condition": {
                    ...
                    "ArnLike": {
                        ...
                        f"myorg-lambda-sechub-findings-report-{account_id}-{region}"
                        )
                    }
                }
            }
...
def create_lambda_function(session, function_name, handler_name, function_role_arn, region):
    ...
    try:
        response = lambda_client.create_function(
            ...
            Layers=[
                f'arn:aws:lambda:{region}:123456789012:layer:AWSSDKPandas-Python311:1'
            ]
        )
...
def configure_scheduler_rule(session, config, lambda_arn):
    ...
    schedule_name = f'myorg-sechub-eb-findings-report-{account_id}-{region}'
...
def main():
    ...
    key_alias = f'alias/myorg-kms-sechub-findings-report-{args.account_id}-{args.region}'
    kms_key_arn = create_kms_key(session, key_alias, args.account_id,
                                 args.region, function_role_arn)

    function_name = f"myorg-lambda-sechub-findings-report-{args.account_id}-{args.region}"
