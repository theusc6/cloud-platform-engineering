...
def create_iam_role(session):
    ...
    role_name = 'role-myorg-awsinspector-findings-report+prod-ops'
    trust_policy = {
        "Version": "2012-10-17",
        "Statement": [
            {
...
def configure_scheduler_rule(session, config):
    ...
    input_json = f"""{{
        ...
        "FilterCriteria": {{
            ...
            {{ "Comparison": "EQUALS", "Value": "CLOSED" }},
            {{ "Comparison": "EQUALS", "Value": "SUPPRESSED" }}
            ]
        }}
    }}"""

    schedule_name = f'myorg-awsinspector-eb-findings-report-{account_id}-{region}'
...
def main():
    ...
    bucket_name = f'myorg-awsinspector-s3-findings-report-{account_id}-{args.region}'
    create_s3_bucket(session, bucket_name, account_id, args.region)

    key_alias = f'alias/myorg-awsinspector-kms-findings-report-{account_id}-{args.region}'
